...
def list_devices():
    ...
    print("\n" + "=" * 50)
    print("Starting automatic tracking for all devices...")
    print("=" * 50)
    
    # Track alle devices automatisch
    for idx, (device_name, canonic_id) in enumerate(canonic_ids, start=1):
        print(f"\n[Tracking] Device {idx}/{len(canonic_ids)}: {device_name}")
        print("-" * 50)
        try:
            get_location_data_for_device(canonic_id, device_name)
        except Exception as e:
            print(f"❌ Error tracking {device_name}: {e}")
        print()
    
    print("\n" + "=" * 50)
    print("✅ All devices tracked successfully!")
    print("=" * 50)
